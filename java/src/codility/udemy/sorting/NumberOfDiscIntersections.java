package codility.udemy.sorting;
...
public class NumberOfDiscIntersections {
    public int solution(int A[]) {
        int len = A.length;
        DiscLog[] discHistory = new DiscLog[len * 2];// As this would have
        int j = 0;
        for (int i = 0; i < len; i++) {
            discHistory[j++] = new DiscLog(i - (long) A[i], 1);// Starting point
            discHistory[j++] = new DiscLog(i + (long) A[i], -1); // Ending point
        }
